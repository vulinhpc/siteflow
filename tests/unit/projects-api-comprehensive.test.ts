...
import { GET, POST, DELETE } from '@/app/api/v1/projects/route';
...
describe('Projects API Comprehensive Tests', () => {
  ...
  describe('POST /api/v1/projects - Create Project', () => {
    ...
    it('should create project with all optional fields', async () => {
      ...
      expect(data.project.description).toBe('This is a comprehensive test project');
      expect(data.project.thumbnailUrl).toBe('https://picsum.photos/400/300?random=1');
    });
    ...
    it('should return 400 for name too long', async () => {
      ...
      expect(response.status).toBe(400);
      expect(data.type).toContain('validation-error');
      expect(data.detail).toContain('Project name must be at most 255 characters');
    });
  ...
  describe('DELETE /api/v1/projects - Soft Delete', () => {
    it('should soft delete project successfully', async () => {
      ...
      mockDb.returning.mockResolvedValueOnce([mockUpdatedProject]);

      const request = new NextRequest('http://localhost:3000/api/v1/projects/project-to-delete', {
        method: 'DELETE',
        headers: {
          'x-e2e-bypass': '1',
          'x-org-id': 'org_e2e_default',
    ...
    it('should not return soft-deleted projects in list', async () => {
      const mockProjects = [
        {
          id: '1',
          name: 'Active Project',
  ...
  describe('Project Status Enum Validation', () => {
    it('should accept all valid status values', async () => {
      const validStatuses = ['PLANNING', 'IN_PROGRESS', 'DONE', 'ON_HOLD', 'CANCELLED'];
